...
namespace application
{
	public class Aggregate
    {
		...
		protected void Publish(IDomainEvent domainEvent)
	    {
		    if (_outbox == null)
		    {
			    _outbox = new HashSet<IDomainEvent>(DomainEventEqualityComparer.Instance);
		    }

		    _outbox.Add(domainEvent);
		}

	    protected void Process<TDomainEvent>(TDomainEvent domainEvent, Action<TDomainEvent> action) where TDomainEvent : IDomainEvent
	    {
		    if (_inbox == null)
		    {
			    _inbox = new HashSet<IDomainEvent>(DomainEventEqualityComparer.Instance);
		    }

		    if (_inbox.Contains(domainEvent)) return;
